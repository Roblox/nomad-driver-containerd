...
| **memory_swap** | string| no | Default ("0"). By default, no swap is used by the container. If you want to use swap, you can set `memory_swap` to a string representing the size of the swap. The host instance will also need to have swap enabled |
| **memory_swappiness** | float | no | Default(0.0). By default, the container will not be swapped out of memory. If you want to swap out of memory, you can set `memory_swappiness` to a float value between 0.0 and 1.0. |
